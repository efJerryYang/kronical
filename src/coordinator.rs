use crate::events::{KeyboardEventData, MouseEventData, WindowFocusInfo, MousePosition, RawEvent};
use crate::focus_tracker::{FocusEventWrapper, FocusChangeCallback};
...
use winshift::{FocusChangeHandler, WindowFocusHook, WindowHookConfig, MonitoringMode};
...
impl FocusChangeCallback for FocusCallback {
    fn on_focus_change(&self, focus_info: WindowFocusInfo) {
        if let Err(e) = self.sender.send(ChronicleEvent::WindowFocusChange { focus_info }) {
...
impl EventCoordinator {
    ...
    fn convert_chronicle_to_raw(event: ChronicleEvent) -> Result<crate::events::RawEvent> {
        ...
        match event {
            ...
            ChronicleEvent::WindowFocusChange { focus_info } => {
                Ok(RawEvent::WindowFocusChange {
                    timestamp: now,
                    event_id,
                    focus_info,
                })
    ...
    pub fn start_main_thread(&self, data_store: Box<dyn StorageBackend>, pid_file: std::path::PathBuf, save_raw: bool) -> Result<()> {
            ...
            thread::spawn(move || {
                ...
                loop {
                    match receiver.recv_timeout(Duration::from_millis(50)) {
                        ...
                        Ok(event) => {
                            ...
                            if let Ok(raw_event) = Self::convert_chronicle_to_raw(event) {
                                let event_memory = std::mem::size_of::<RawEvent>() + match &raw_event {
                                    RawEvent::WindowFocusChange { focus_info, .. } => {
                                        focus_info.app_name.len() + focus_info.window_title.len()
                                    }
                                    RawEvent::KeyboardInput { data, .. } => {
                                        data.modifiers.iter().map(|s| s.len()).sum::<usize>() +
                                        data.key_char.map_or(0, |_| std::mem::size_of::<char>())
                                    }
                                    _ => 0
                                };
                        ...
                        Err(mpsc::RecvTimeoutError::Timeout) => {
                            if !pending_raw_events.is_empty() {
                                info!("Step 5b: Processing {} pending raw events", pending_raw_events.len());
                                
                                let raw_events_to_process = std::mem::take(&mut pending_raw_events);
                                total_raw_events_processed += raw_events_to_process.len();

                                let new_records = record_processor.process_events(raw_events_to_process.clone());
                                if !new_records.is_empty() {
                                    ...
                                    if let Err(e) = store.add_records(new_records) {
                                        ...
                                        socket_server_clone.update_records(recent_records.iter().cloned().collect());
                                    }
                                }

                                match compression_engine.compress_events(raw_events_to_process) {
                                    Ok((processed_events, compact_events)) => {
                                        info!("Step 5b: Compressed into {} compact events", compact_events.len());
                                        
                                        if !compact_events.is_empty() {
                                            if let Err(e) = store.add_compact_events(compact_events) {
                                                error!("Step 5b: Failed to store compact events: {}", e);
                                            }
                                        }

                                        if save_raw && !processed_events.is_empty() {
                                            if let Err(e) = store.add_events(processed_events) {
                                                error!("Step 5b: Failed to store raw events: {}", e);
                                            }
                                        }
                                    }
                                    Err(e) => {
                            ...
                            if last_metric_update.elapsed() >= Duration::from_secs(1) {
                                let records_memory_mb = recent_records.iter().map(|r| std::mem::size_of_val(r)).sum::<usize>() as f32 / 1024.0 / 1024.0;
                                let mut compression_info = compression_engine.get_compression_info(total_raw_events_processed, total_raw_events_memory_bytes);
                    ...
                    if let Some(timeout_record) = record_processor.check_timeouts() {
                        info!("Step T: Timeout detected, storing record: {:?}", timeout_record.state);
