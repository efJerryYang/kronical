...
fn main() {
    eprintln!("winshift-repro is intended for macOS only");
    std::process::exit(1);
}
...
mod macos_repro {
    ...
    #[derive(Parser, Debug)]
    #[command(name = "winshift-repro")]
    #[command(about = "Manual memory-growth repro harness for winshift on macOS")]
    struct Args {
        /// Which suspicious path to stress. `hook-cycle` is the primary target.
        #[arg(long, value_enum, default_value_t = Mode::HookCycle)]
        mode: Mode,

        /// Number of iterations/cycles to run. Use 0 to run until interrupted.
        #[arg(long, default_value_t = 200)]
        iterations: u64,

        /// Print one sample line every N iterations.
        #[arg(long, default_value_t = 10)]
        sample_every: u64,

        /// Monitoring mode used by hook-cycle.
        #[arg(long, value_enum, default_value_t = Monitor::Combined)]
        monitor: Monitor,

        /// Whether hook-cycle should request embedded ActiveWindowInfo.
        #[arg(long, default_value_t = true, action = clap::ArgAction::Set)]
        embed_active_info: bool,

        /// How long each hook-cycle run stays active before attempting stop.
        #[arg(long, default_value_t = 250)]
        hook_run_ms: u64,

        /// Sleep between hook cycles.
        #[arg(long, default_value_t = 50)]
        cooldown_ms: u64,

        /// Sleep between active-window queries.
        #[arg(long, default_value_t = 10)]
        query_sleep_ms: u64,
    }
    ...
    fn print_sample(label: &str, iteration: u64, stats: &Arc<CallbackStats>, run_stats: &RunStats) {
